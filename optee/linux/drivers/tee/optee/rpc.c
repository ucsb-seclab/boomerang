...
static int try_mapping_user_pages(u32 session_id, struct optee_context_data *ctxdata, void *buffer, size_t size, bool is_write, void **target_p_addr, void **target_p_addr1) {
 	struct optee_session *target_session;
 	int ret = 0;
 	target_session = find_session(ctxdata, session_id);
 	if(target_session) {
 		ret = boomerang_prep_mem_buffer(buffer, size, is_write, target_session, target_p_addr, target_p_addr1);
 		//pr_warning("BOOMERANG: Amazing...we found the session\n");
 	} else {
 		pr_warning("BOOMERANG:BAD BAD..No session found\n");
 	}
 	return ret;
				...
				struct tee_shm *shm)
{
	...
	case OPTEE_MSG_RPC_CMD_SHM_FREE:
		...
		break;
	case OPTEE_SMC_RETURN_PP_CHECK:
 		// This is just a check
 		//params[1].u.value.a = 0;
 		//params[1].u.value.b = params[0].u.value.a;
 		//params[1].u.value.c = params[0].u.value.b;
 		//break;
 	case OPTEE_SMC_RETURN_PP_BUFVERIFY:
 		// set default value as failure
 		#ifdef PPTIME
 		start = ktime_get();
 		#endif
 		params[1].u.value.a = -1;
 		if(params[0].u.value.c == current->pid) {
 			is_writable = params[1].u.value.c > 0;
 			target_p_addr = NULL;
 			target_p_addr1 = NULL;
 			if(!try_mapping_user_pages((u32)params[1].u.value.b, (struct optee_context_data *)ctx->data, params[0].u.value.a, params[0].u.value.b, is_writable, &target_p_addr, &target_p_addr1)) {
 				// success return value
 				params[1].u.value.a = 0;				
 				params[1].u.value.b = target_p_addr;
 				params[1].u.value.c = target_p_addr1;
 				#ifdef PPTIME
 				end = ktime_get();
 				actual_time = ktime_to_ns(ktime_sub(end, start));
 				pr_warning("BOOMERANG: USER PAGE PINNING TIME=%lld ns\n", actual_time);
 				#endif
 				//pr_warning("BOOMERANG %s: Returning PA:%p for VA:%p\n", __func__, target_p_addr, (void*)params[0].u.value.a);
 			} else {
 				pr_warning("BOOMERANG %s: UNABLE TO MAP PROVIDED ADDRESS\n", __func__);
 			}
 		} else {
 			pr_warning("BOOMERANG %s: BAD BAD...PID mismatch\n", __func__);
 		}
 		break;
	default:
