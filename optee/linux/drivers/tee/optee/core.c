			 ...
			 const struct optee_msg_param *msg_params)
{
	...
	for (n = 0; n < num_params; n++) {
		...
		case OPTEE_MSG_ATTR_TYPE_TMEM_INOUT:
			...
			break;
		//BOOMERANG
 		case OPTEE_MSG_ATTR_TYPE_PP_INPUT:
 			p->attr = TEE_IOCTL_PARAM_ATTR_TYPE_PP_MEMREF_INPUT +
 				  attr - OPTEE_MSG_ATTR_TYPE_PP_INPUT;
 			break;
 		case OPTEE_MSG_ATTR_TYPE_PP_OUTPUT:
 		case OPTEE_MSG_ATTR_TYPE_PP_INOUT:
 			//pr_warning("BOOMERANG FROM SECURE WORLD:0x%llx\n", (u64)mp->u.tmem.size);
 			p->u.memref.size = mp->u.tmem.size;
 			break;
		default:
			return -EINVAL;
		       ...
		       const struct tee_param *params)
{
	...
	for (n = 0; n < num_params; n++) {
		...
		case TEE_IOCTL_PARAM_ATTR_TYPE_MEMREF_INOUT:
			...
			break;
		//BOOMERANG
 		case TEE_IOCTL_PARAM_ATTR_TYPE_PP_MEMREF_INPUT:
 		case TEE_IOCTL_PARAM_ATTR_TYPE_PP_MEMREF_OUTPUT:
 		case TEE_IOCTL_PARAM_ATTR_TYPE_PP_MEMREF_INOUT:
 			mp->attr = OPTEE_MSG_ATTR_TYPE_PP_INPUT +
 				   p->attr -
 				   TEE_IOCTL_PARAM_ATTR_TYPE_PP_MEMREF_INPUT;
 			mp->u.tmem.shm_ref = (unsigned long)p->u.memref.shm;
 			mp->u.tmem.size = p->u.memref.size;
 			mp->u.tmem.buf_ptr = (unsigned long)p->u.memref.shm;
 			//pr_warning("BOOMERANG: passing:%p\n", mp->u.tmem.buf_ptr);
 			mp->attr |= OPTEE_MSG_ATTR_CACHE_PREDEFINED <<
 					OPTEE_MSG_ATTR_CACHE_SHIFT;
 			break;
		default:
