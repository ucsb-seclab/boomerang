...
u32 optee_do_call_with_arg(struct tee_context *ctx, phys_addr_t parg)
{
	...
	while (true) {
		...
		//BOOMERANG: This is needed so that
		// CSR can verify memory requests.
		pid_t cpid = current->pid;
		optee->invoke_fn(param.a0, param.a1, param.a2, param.a3,
				 param.a4, param.a5, cpid, param.a7,
				 &res);
		       ...
		       struct tee_param *param)
{
	...
	INIT_LIST_HEAD(&(sess->boomerang_shmem_list));
...
int optee_close_session(struct tee_context *ctx, u32 session)
{
	...
	if (sess) {
		//BOOMERANG
		boomerang_release_mem_buffers(&(sess->boomerang_shmem_list));
